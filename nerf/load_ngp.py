...
pyd_dirs += [os.path.dirname(pyd) for pyd in glob.iglob(os.path.join(NGP_DIR, "out/build*", "**/*.pyd"), recursive=True)]
pyd_dirs += [os.path.dirname(pyd) for pyd in glob.iglob(os.path.join(NGP_DIR, "out/build*", "**/*.so"), recursive=True)]
